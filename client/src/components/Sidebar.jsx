    ...
    text-white ${selectedUser ? "max-md:hidden" : " "}`} >
        <div className="pb-5">
            ...
            <div className="bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5">
                ...
                <input type="text" className="bg-transparent border-none outline-none w-5
        ...
        <div className="flex flex-col">
            {userDummyData.map((user,index)=>(
                ...
                cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                    ...
                    className="w-[35px] aspect-[1/1] rounde-full" />
                    ...
                    {index > 2 && <p className="absolute top-400 right-4 text-xs h-5 w-5
                    flex justify-center items-center rounded-full bg-violate-500/50">{index}</p>}
                </div>
            ))}
        </div>
    </div>
